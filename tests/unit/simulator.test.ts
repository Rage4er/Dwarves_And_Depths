...
import { Dwarf, Enemy, BattleConfig, Role, Rarity, Position, Tag, Equipment, StatusEffect } from '../../src/core/types';

// Helper: создание гнома с полными полями §2.3
function createDwarf(overrides: Partial<Dwarf> = {}): Dwarf {
  const base: Dwarf = {
    id: 'd1',
    name: 'Test Dwarf',
    hp: 100,
    maxHp: 100,
    role: 'warrior' as Role,
...
describe('BattleSimulator', () => {
  ...
  it('should end battle when all enemies defeated (гном vs крыса e_rat)', () => {
    ...
    const config: BattleConfig = { ...defaultConfig, maxRounds: 20 };
  ...
  it('should end battle when hero dies (heroDeathEndsBattle=true)', () => {
    ...
    const config: BattleConfig = { ...defaultConfig, heroDeathEndsBattle: true, maxRounds: 5 };
  ...
  it('should reach max rounds and end in draw', () => {
    ...
    const config: BattleConfig = { ...defaultConfig, maxRounds: 3, heroDeathEndsBattle: false };
