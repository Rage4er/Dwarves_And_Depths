...
import { Dwarf, Enemy, BattleState, BattleConfig, StatusEffect } from '../core/types';

export class BattleSimulator {
  private state: BattleState;

  constructor(heroes: Dwarf[], enemies: Enemy[], config: BattleConfig) {
    this.state = {
      round: 1,
      heroes,
      enemies,
  ...
  simulateRound(): void {
    ...
    const allUnits = [
      ...this.state.heroes.map(h => ({ unit: h, side: 'hero' as const })),
      ...this.state.enemies.map(e => ({ unit: e, side: 'enemy' as const })),
    ...
    for (const { unit, side } of allUnits) {
      ...
      if (side === 'hero') {
        this.heroAttack(unit as Dwarf);
      } else {
        this.enemyAttack(unit as Enemy);
      }
    }
  ...
  private applyStatusEffects(unit: Dwarf | Enemy): void {
    if (!('statusEffects' in unit) || !unit.statusEffects) {
      return;
    }

    const dwarf = unit as Dwarf;
  ...
  private heroAttack(hero: Dwarf): void {
  ...
  simulateUntilEnd(maxRoundsOverride?: number): BattleState {
